...
<template>
  <div class="flex justify-center items-center min-h-screen p-8 bg-gradient-to-br from-primary to-secondary overflow-hidden">
    <div
    ...
    >
      ...
      <div class="relative z-10">
        <div class="flex items-center mb-6">
          <div v-if="props.user.picture" class="w-20 h-20 rounded-full overflow-hidden mr-4 border-2 border-accent shadow-md">
            <img :src="props.user.picture" :alt="props.user.name" class="w-full h-full object-cover" />
          </div>
          <div v-else class="w-20 h-20 rounded-full overflow-hidden mr-4 border-2 border-accent shadow-md bg-secondary flex items-center justify-center text-2xl font-bold text-accent">
            {{ getInitials }}
          </div>
          <div>
            <h2 class="text-2xl font-bold text-accent">{{ props.user.name || props.user.nickname }}</h2>
            <p class="text-sm text-secondary">{{ props.user.email }}</p>
          </div>
        </div>
        <div class="space-y-4">
          <div v-if="props.user.email_verified" class="flex items-center text-success">
          ...
          <div class="flex items-center text-info">
            ...
            <span>Last login: {{ new Date(props.user.updated_at!).toLocaleString() }}</span>
          </div>
        </div>
        <button
          @click="props.logout"
